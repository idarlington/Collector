...
class TileSystem {
  ...
  def latLongToPixelXY(latitude: Double,
                       longitude: Double,
                       levelOfDetail: Int): (Int, Int) = {
    val _latitude = clip(latitude, MinLatitude, MaxLatitude)
    ...
    val y
    : Double = 0.5 - Math.log((1 + sinLatitude) / (1 - sinLatitude)) / (4 * Math.PI)
  ...
  def pixelXYToLatLong(pixelX: Int,
                       pixelY: Int,
                       levelOfDetail: Int): (Double, Double) = {
